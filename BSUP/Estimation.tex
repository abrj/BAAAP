...
Delphi:
\begin{itemize}
    \item Only the average estimate is shown which creates no direct incentive for discussions
\end{itemize}

Planning Poker:
\begin{itemize}
    \item All estimates are shown, which can be negative if some team members try to overrule the others
    \item Inexperience is a factor that might produce several wrong estimates if it involves the entire team
    \item Can be time consuming
\end{itemize}

To conclude, our experience with Planning Poker and Delphi was quite good. They both take some time to complete and learn. But the fact that especially with Planning Poker, all team members get a very good idea of what each task exactly consists of, is is something we find very valuable to remember for future projects.
...
This is done by distributing the probability across the time frame, somehow resembling a \textit{normal distribution}. Cumulating between two time estimates, can then give you the probability of how likely the task is to finish sometime within the timespan of the two estimates as seen in figure \ref{TynerBlain_figure}. \


\begin{figure}[H]
\centering
\includegraphics[scale=0.5]{PERTDistribution.png} 
\caption{Nearly a hundred percent chance the task will finish sometime between 2 and 6 hours \protect\cite{TynerBlain_graph_w}}
\label{TynerBlain_figure}
\end{figure}
...
For our purpose some general distribution for this is though used in many projects and gives a pretty good picture of how these estimates can be used. These are taken from the website, which can be found in appendix \cite{3point_w}\
\begin{itemize}
...
\textbf{Pros:}
\begin{itemize}
\item If the estimators possess a decent amount of knowledge and experience within the subjects of the tasks, very reliable estimates can be given. 
\end{itemize}


\textbf{Cons:}
\begin{itemize}
\item This technique can be difficult to use in the beginning phase of a project, since it is not always here that all necessary information is available.
\item The direct estimation technique can be very time consuming, and thereby costly, to use.
\item If the estimators are inexperienced the time and resources used on estimation can possibly be wasted
\end{itemize}


This finishes the section for our third technique used in the last sprint. We will proceed with a comparison of the techniques and outline the aspects of them.  
